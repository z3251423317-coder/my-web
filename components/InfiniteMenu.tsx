...
class InfiniteGridMenu {
  ...
  #generateSpherePoints(count: number, radius: number): vec3[] {
    const points: vec3[] = [];
    if (count <= 1) {
      // Align single item directly facing the camera snaps at [0, 0, -radius]
      return [vec3.fromValues(0, 0, -radius)];
    }
    
    if (count <= 4) {
      // Place small number of items evenly distributed on the horizontal equator circle (XZ plane)
      for (let i = 0; i < count; i++) {
        const angle = (i * 2 * Math.PI) / count;
        const x = Math.cos(angle) * radius;
        const z = Math.sin(angle) * radius;
        points.push(vec3.fromValues(x, 0, z));
      }
      return points;
    }
    
    // For 5 or more items, use Fibonacci sphere distribution for a gorgeous 3D distribution
    ...
    for (let i = 0; i < count; i++) {
      ...
      points.push(vec3.fromValues(x * radius, y * radius, z * radius));
    }
    return points;
  }

  updateItems(newItems: InfiniteMenuItem[]) {
    this.items = newItems || [];
    ...
    if (this.gl) {
      ...
      this.instancePositions = this.#generateSpherePoints(count, this.SPHERE_RADIUS);
      ...
      if (this.control) {
        ...
        this.control.snapDirection = vec3.fromValues(0, 0, -1);
        this.control.snapTargetDirection = vec3.fromValues(0, 0, -1);
      }
    }
  }
...
const defaultItems = [
  {
    ...
    title: 'Default Card',
    description: 'A system placeholder'
  }
];
...
export default function InfiniteMenu({ items = [], scale = 1.0, onItemClick }: InfiniteMenuProps) {
  ...
  useEffect(() => {
    ...
    if (itemsStr !== prevItemsStrRef.current) {
